...
- githubremote:    Add git remote for FireflyUI Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/FireflyUI-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop com.fireflyui.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/FiireflyUI/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop com.fireflyui.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop com.fireflyui.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
